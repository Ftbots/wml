...
class WZMLStyle:
    """
    A class containing styling information for the Telegram bot.
    """
    ...
    TOKEN_MSG = f'''<b><u>Generated Temporary Login Token!</u></b>
...
<a href="https://t.me/Nxleech"><b>♥️ 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 𝗡𝘅𝗟𝗲𝗲𝗰𝗵</b></a>
    '''
    REPO_STATS = '''<b>🧑‍💻 <u>𝙍𝙚𝙥𝙤 𝙎𝙩𝙖𝙩𝗶𝘀𝘁𝗶𝗰𝘀</u></b>
...
<a href="https://t.me/Nxleech"><b>♥️ 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 𝗡𝘅𝗟𝗲𝗲𝗰𝗵</b></a>
    ...
    RESTART_SUCCESS = f'''<b>♻️ Restarted Successfully!</b>
...
------------------------------------------\n\n'''
    ...
    ####--------OVERALL MSG HEADER----------
    STATUS_NAME =       '┏🏷️ File: <code>{Name}</code> ⬇️' # Modified

    #####---------PROGRESSIVE STATUS-------
    BAR =               '\n┠[{Bar}] {Percentage}%' # Modified: added percentage
    PROCESSED =         '\n┠<b>🔄 Progress:</b> <code>{Processed}</code> of <code>{Size}</code> ({Speed})' # Modified: Combined Processed and Speed
    STATUS =            '\n┠<b>Status:</b> <a href="{Url}">{Status}</a>'  #Modified: removed emoji and <a>
    ETA =               '' # Disabled ETA
    SPEED =             '' # Disabled Speed, because it's in PROCESSED
...
# Example usage demonstrating HTML escaping
user_file_name = "<script>attack!</script>ImportantFile.txt"
safe_file_name = escape_html(user_file_name)

# Correct and secure usage within your style class:
WZMLStyle.NAME = f'┏<b>🏷️ Name:</b> <code>{safe_file_name}</code>\n'
